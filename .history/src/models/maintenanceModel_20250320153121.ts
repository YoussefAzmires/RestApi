...
async function initialize(): Promise<void> {
  try {
    // Use env variable if available; otherwise, fallback to hardcoded URI
    const MONGODB_URI =
    ...
    if (collectionArray.length == 0) {
      await db.createCollection("maintenance", { collation: { locale: "en", strength: 1 } });
    }

    maintenanceCollection = db.collection("maintenance");
  } catch (err) {
    console.error(
      err instanceof MongoError ? `MongoDB connection failed: ${err.message}` : `Unexpected error: ${err}`
    );
  }
}
